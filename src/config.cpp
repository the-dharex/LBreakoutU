...
Config::Config()
{
	...
	if (dname != ".") { /* . = disabled install */
		if (dname.length() > 0) dname = getHomeDir() + "/" + dname;
		else dname = getHomeDir();
		if (!dirExists(dname)) {
...
void Config::save()
{
	...
	ofs << "bcc_type=" << bcc_type << "\n";
		...
		if (wiiu_save_file(rel, ofs.str())) {
			_loginfo("Configuration saved to %s\n",path.c_str());
		} else {
			_logerr("Failed to save config via wiiu_save_file\n");
		}
		return;
	}
#else
