...
extern SDL_Texture *globalTarget;
void MyRenderPresent();
#define SDL_RenderPresent(x) MyRenderPresent()

// On Wii U, the GX2 swizzler skips completely transparent pixels (0x00000000).
inline SDL_Texture* WIIU_CreateTextureFromSurface(SDL_Renderer* renderer, SDL_Surface* surface) {
    if (!surface || !surface->pixels) return NULL;
    
    if (surface->format->BytesPerPixel == 4) {
        SDL_Texture* tex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, surface->w, surface->h);
        if (tex) {
            ...
            if (SDL_LockTexture(tex, NULL, &pixels, &pitch) == 0) {
                ...
                SDL_memset(pixels, 0, pitch * surface->h);
                
                for (int y = 0; y < surface->h; y++) {
                    Uint32* src_row = (Uint32*)((Uint8*)surface->pixels + y * surface->pitch);
                    Uint32* dst_row = (Uint32*)((Uint8*)pixels + y * pitch);
                    for (int x = 0; x < surface->w; x++) {
                        Uint8 r, g, b, a;
                        SDL_GetRGBA(src_row[x], surface->format, &r, &g, &b, &a);
                        if (a == 0) {
                            a = 1; r = 0; g = 0; b = 0; // Force A=1, RGB=0
                        }
                        // Manually pack RGBA8888 (Big Endian memory layout: R is byte 0, A is byte 3)
...
#define SDL_CreateTextureFromSurface WIIU_CreateTextureFromSurface
#endif
...
/*
 * Safe replacement for SDL_FlushEvents(SDL_FIRSTEVENT, SDL_LASTEVENT)
 * Flushes only input events so we don't drop SDL_QUIT or SDL_APP_WILLENTERBACKGROUND
 */
inline void FlushInputEvents() {
...
/* 
 * Replaces 'y/n' with 'A/B' in strings for Wii U
 */
inline string wiiu_confirm_str(string s) {
#ifdef __WIIU__
	size_t pos = s.find("y/n");
	if (pos != string::npos) s.replace(pos, 3, "A/B");
#endif
	return s;
}

/* 
 * Flushes events that can easily clog the single-event queue mechanism
 */
inline void FlushUselessEvents() {
...
public:
	...
	int opened() {
#ifdef __WIIU__
        return 1; // Native input is always available
#else
		return (js != NULL);
#endif
	}
...
void runWaitDialog(Font &font, const string &caption);
