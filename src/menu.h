...
class MenuItemGamepadKey : public MenuItemValue {
	int &btn_offset;
	bool waitForNewKey;
public:
	MenuItemGamepadKey(const string &c, const string &tt, int &_btn_offset)
			: MenuItemValue(c,"",tt,AID_CHANGEKEY),
			  btn_offset(_btn_offset), waitForNewKey(false) {
		updateText();
	}
	void updateText() {
		const char *names[] = {"A", "B", "X", "Y", "L", "R", "ZL", "ZR", "+", "-"};
		if (btn_offset >= 0 && btn_offset < 10) {
			setValue(names[btn_offset]);
		} else {
			setValue("?");
		}
	}
