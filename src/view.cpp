	  ...
	  fpsCycles(0), fpsStart(0), fps(0)
{
	...
	/* load theme names */
#ifdef __WIIU__
    // Dump romfs contents to find where the files actually are!
    _sd_log("\n--- DUMPING romfs:/ ---\n");
    DIR *d = opendir("romfs:/");
    if (d) {
        struct dirent *ent;
        ...
        _sd_log("Failed to open romfs:/\n");
    }

    _sd_log("\n--- DUMPING romfs:/content/ ---\n");
    d = opendir("romfs:/content/");
    if (d) {
        struct dirent *ent;
        while ((ent = readdir(d)) != NULL) {
            _sd_log("romfs:/content/ -> %s\n", ent->d_name);
        }
        closedir(d);
    } else {
        _sd_log("Failed to open romfs:/content/\n");
    }
    ...
    if (d) {
        struct dirent *ent;
        while ((ent = readdir(d)) != NULL) {
            _sd_log("/vol/content/ -> %s\n", ent->d_name);
        }
        closedir(d);
    } else {
        _sd_log("Failed to open /vol/content/\n");
    }

    // Hardcode theme names since readDir/stat might be buggy on WUT romfs
    themeNames.push_back("Classic");
    themeNames.push_back("Standard");
#else
	readDir(string(DATADIR)+"/themes", RD_FOLDERS, themeNames);
#endif
	if (string(CONFIGDIR) != ".") {
		vector<string> homeThemes;
		string configBase = getHomeDir();
#ifdef __WIIU__
		if (config.player_names[0] == "Michael") {
			int16_t miiName[12] = {0};
			if (nn::act::GetMiiName(miiName) == 0) { // nn::Result 0 is usually OK
				char utf8Name[12] = {0};
				int j = 0;
				for (int i=0; i<10 && miiName[i] != 0; i++) {
					if ((miiName[i] & 0xFF) >= 32 && (miiName[i] & 0xFF) <= 126) {
						utf8Name[j++] = (char)(miiName[i] & 0xFF);
					}
				}
				if (j > 0) {
					config.player_names[0] = utf8Name;
					config.save();
				}
			}
		}
#endif
		if (string(CONFIGDIR).length() > 0) configBase += "/" + string(CONFIGDIR);
		readDir(configBase + "/themes", RD_FOLDERS, homeThemes);
	...
	if (pos < themeNames.size()) {
		...
		if (!themeNames.empty()) {
			...
			_logerr("No themes found! Please install the game data to sd:/lbreakouthd\n");
			_sd_log("No themes found, adding Standard dummy theme\n");
			/* We add a dummy theme so it doesn't crash later, though the game will likely fail to load graphics */
			themeNames.push_back("Standard");
		}
	}
...
void View::run()
{
	...
	while (!leave && !quitReceived) {
		...
		if (!newEvent || ev.type != SDL_QUIT) {
			...
			if (gpadstate[GPAD_BUTTON0 + config.gp_pause] && !prev_gpadstate[GPAD_BUTTON0 + config.gp_pause]) code = SDL_SCANCODE_P;
			else if (gpadstate[GPAD_BUTTON0 + config.gp_escape] && !prev_gpadstate[GPAD_BUTTON0 + config.gp_escape]) code = SDL_SCANCODE_ESCAPE;
			...
			if (ev.type == SDL_KEYUP) {
				...
				case SDL_SCANCODE_R:
					if (cgame.getCurrentPlayer()->getLives() > 1 ) {
						text.clear();
						text.push_back(wiiu_confirm_str(_("Restart level? y/n")));
				...
				case SDL_SCANCODE_ESCAPE:
					text.clear();
					text.push_back(wiiu_confirm_str(_("Quit Game? y/n")));
		...
		if (1) {
			...
			if (gpadstate[GPAD_RIGHT])
				pis.right = 2; // In classic LBreakoutHD this was 2, let's keep it consistent
		...
		if (flags & CGF_LASTLIFELOST) {
			...
			text.push_back(wiiu_confirm_str(_("Buy Continue? y/n")));
...
#else
	mControls->add(new MenuItemGamepadKey(_("Left Fire"),
			_("Fire attached ball. Use paddle weapon."), config.gp_lfire));
	mControls->add(new MenuItemGamepadKey(_("Right Fire"),
			_("Use paddle weapon."), config.gp_rfire));
	mControls->add(new MenuItemGamepadKey(_("Paddle Turbo"),
			_("Move paddle faster."), config.gp_turbo));
	mControls->add(new MenuItemGamepadKey(_("Ball Turbo"),
			_("Hold to speed up balls. Great to skip wait times."), config.gp_warp));
	mControls->add(new MenuItemGamepadKey(_("Max Ball Speed"),
			_("Speed up all balls to the maximum immediately."), config.gp_maxballspeed));
	mControls->add(new MenuItemGamepadKey(_("Pause"),
			_("Pause the game."), config.gp_pause));
	mControls->add(new MenuItemGamepadKey(_("Quit Game"),
			_("Quit or abort the current game."), config.gp_escape));
#endif
#ifndef __WIIU__
	...
	while (!quitReceived) {
		...
		if (SDL_PollEvent(&ev)) {
			...
			if (ev.type == SDL_QUIT)
				quitReceived = true;
			if (changingKey) {
				MenuItemKey *keyItem = dynamic_cast<MenuItemKey*>(curMenu->getCurItem());
				if (keyItem && ev.type == SDL_KEYDOWN) {
					switch (ev.key.keysym.scancode) {
					case SDL_SCANCODE_P:
						break;
					case SDL_SCANCODE_ESCAPE:
		...
		if (changingKey) {
#ifdef __WIIU__
			MenuItemGamepadKey *gpadItem = dynamic_cast<MenuItemGamepadKey*>(curMenu->getCurItem());
			if (gpadItem) {
				int pressed_btn = -1;
				bool any_pressed = false;
				for (int i = 0; i < 10; i++) {
					if (gpadstate[GPAD_BUTTON0 + i]) {
						any_pressed = true;
						pressed_btn = i;
						break;
					}
				...
				if (waiting_for_release) {
					...
					gpadItem->setKey(pressed_btn);
					changingKey = false;
					changedKey = true;
					gpad_delay = 12;
				} else if (gpadstate[GPAD_LEFT] || gpadstate[GPAD_RIGHT] || gpadstate[GPAD_UP] || gpadstate[GPAD_DOWN]) {
					// cancel on d-pad
					gpadItem->cancelChange();
					changingKey = false;
					gpad_delay = 12;
				}
			}
...
#ifdef __WIIU__
		if (!changingKey && (!newEvent || ev.type != SDL_QUIT)) {
			SDL_Scancode code = SDL_SCANCODE_UNKNOWN;
			
			// A and B buttons must be JUST PRESSED
			...
			// D-Pad allows repeating with delay
			if (gpadstate[GPAD_LEFT] && !prev_gpadstate[GPAD_LEFT]) { code = SDL_SCANCODE_LEFT; gpad_delay = 25; }
			else if (gpadstate[GPAD_RIGHT] && !prev_gpadstate[GPAD_RIGHT]) { code = SDL_SCANCODE_RIGHT; gpad_delay = 25; }
			else if (gpadstate[GPAD_UP] && !prev_gpadstate[GPAD_UP]) { code = SDL_SCANCODE_UP; gpad_delay = 25; }
				...
				if (gpadstate[GPAD_LEFT]) { code = SDL_SCANCODE_LEFT; gpad_delay = 6; }
				else if (gpadstate[GPAD_RIGHT]) { code = SDL_SCANCODE_RIGHT; gpad_delay = 6; }
				else if (gpadstate[GPAD_UP]) { code = SDL_SCANCODE_UP; gpad_delay = 6; }
...
#ifdef __WIIU__
	if (saveFileName.find("/vol/save") == 0) {
		stringstream ofs;
		...
		if (wiiu_save_file(saveFileName.substr(9), ofs.str())) {
			_loginfo("Game saved to %s\n",saveFileName.c_str());
		} else {
			_logerr("Failed to save game via wiiu_save_file\n");
		}
		return;
	}
#endif
...
int View::waitForKey(int type)
{
	...
	while (!leave) {
		...
		static Uint8 prev_gpadstate[GPAD_LAST1] = {0};
		const Uint8 *gpadstate = gamepad.update();
		if (SDL_PollEvent(&ev)) newEvent = true;
		if (!newEvent || ev.type != SDL_QUIT) {
			SDL_Scancode code = SDL_SCANCODE_UNKNOWN;
			if (gpadstate[GPAD_BUTTON0 + config.gp_pause] && !prev_gpadstate[GPAD_BUTTON0 + config.gp_pause]) code = SDL_SCANCODE_P;
			else if (gpadstate[GPAD_BUTTON0 + config.gp_escape] && !prev_gpadstate[GPAD_BUTTON0 + config.gp_escape]) code = SDL_SCANCODE_ESCAPE;
			else if (gpadstate[GPAD_BUTTON0 + 0] && !prev_gpadstate[GPAD_BUTTON0 + 0]) code = SDL_SCANCODE_RETURN; // A button
			else if (gpadstate[GPAD_BUTTON0 + 1] && !prev_gpadstate[GPAD_BUTTON0 + 1]) code = SDL_SCANCODE_ESCAPE; // B button
			if (code != SDL_SCANCODE_UNKNOWN) {
				ev.type = SDL_KEYUP;
				ev.key.keysym.scancode = code;
...
#ifdef __WIIU__
	string helpText = _(
		"You can control your paddle either with the touch screen, D-pad or left analog stick. "
		...
		"  - PLUS to leave the game (you'll have to confirm)\n"
		"  - L to warp to next level (if enough bricks were destroyed)\n"
		"  - MINUS to pause/unpause the game\n"
		"  - TOUCH/ZL to destroy a brick (workaround for bad level design)\n\n"
...
void View::runBrickDestroyDlg()
{
	...
	while (!leave) {
		...
		if (!newEvent || ev.type != SDL_QUIT) {
			SDL_Scancode code = SDL_SCANCODE_UNKNOWN;
			if (gpadstate[GPAD_BUTTON0 + 0] && !prev_gpadstate[GPAD_BUTTON0 + 0]) code = SDL_SCANCODE_RETURN; // A button -> YES
			else if (gpadstate[GPAD_BUTTON0 + 1] && !prev_gpadstate[GPAD_BUTTON0 + 1]) code = SDL_SCANCODE_ESCAPE; // B button -> NO
			if (code != SDL_SCANCODE_UNKNOWN) {
				ev.type = SDL_KEYUP;
				ev.key.keysym.scancode = code;
				newEvent = true;
