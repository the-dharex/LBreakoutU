...
bool Theme::isOldTheme(const string &name) {
	...
	if (name[0] == '~') {
		path = getHomeDir();
		if (string(CONFIGDIR).length() > 0) path += "/" + string(CONFIGDIR);
		path += "/themes/" + name.substr(1);
	}
	else
				...
				int antialiasing)
{
    ...
    _sd_log("Theme::load: trying to load theme from %s\n", path.c_str());
	...
	if (!fileExists(path + "/theme.ini")) {
        _sd_log("Theme::load: theme.ini NOT FOUND at %s/theme.ini\n", path.c_str());
		if (oldTheme) {
        ...
        _sd_log("Theme::load: falling back to %s/theme.ini\n", stdPath.c_str());
	} else {
        _sd_log("Theme::load: theme.ini FOUND\n");
        if (oldTheme)
            stdPath = string(DATADIR) + "/themes/Classic";
        else
            stdPath = string(DATADIR) + "/themes/Standard";
