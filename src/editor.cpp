...
void Editor::init(const string &setname) {
	SDL_Texture *old;
	SDL_Color g1 = {20,20,20,255}, g2 = {30,30,30,255};
	...
	// Outer frame
	selFrame.fill(0,0,bw,1,250,200,0,255);
	selFrame.fill(0,bh-1,bw,1,250,200,0,255);
	selFrame.fill(0,1,1,bh-2,250,200,0,255);
	selFrame.fill(bw-1,1,1,bh-2,250,200,0,255);
	
	// Inner frame
	selFrame.fill(1,1,bw-2,1,255,125,0,255);
	selFrame.fill(1,bh-2,bw-2,1,255,125,0,255);
	selFrame.fill(1,2,1,bh-4,255,125,0,255);
	selFrame.fill(bw-2,2,1,bh-4,255,125,0,255);
...
void Editor::load()
{
	...
	if (fpath.find("/vol/save") == 0) {
		...
		if (wiiu_load_file(rel, content)) {
			stringstream ifs(content);
			_logdebug(1,"Loading levelset %s (via wiiu_load_file)\n",fpath.c_str());
			levels.clear();
			numLevels = 0;
			hasChanges = false;
			readLine(ifs,line);
			if (line.find("Version:") == string::npos) goto failure_wiiu;
			version = line.substr(9);
			while (readLine(ifs,line)) {
				if (line != "Level:") goto failure_wiiu;
				EditorLevel n;
				readLine(ifs,n.author);
				readLine(ifs,n.title);
				readLine(ifs,line); if (line != "Bricks:") goto failure_wiiu;
				for (uint y = 0; y < EDIT_HEIGHT; y++) {
					readLine(ifs,line);
					for (uint x = 0; x < EDIT_WIDTH; x++)
						n.bricks[x][y] = brickChar2Id(line[x]);
				}
				readLine(ifs,line); if (line != "Bonus:") goto failure_wiiu;
				for (uint y = 0; y < EDIT_HEIGHT; y++) {
					readLine(ifs,line);
					for (uint x = 0; x < EDIT_WIDTH; x++)
						n.extras[x][y] = extraChar2Id(line[x]);
				}
				levels.push_back(n);
				numLevels++;
			}
...
#ifdef __WIIU__
	/* Create SD directories on demand, only when actually saving */
	{
		const char* sdRoot = WHBGetSdCardMountPath();
		string base = string(sdRoot ? sdRoot : "fs:/vol/external01");
		makeDir(base + "/lbreakouthd");
		makeDir(base + "/lbreakouthd/levels");
	}
#endif
...
void Editor::handleButton(uint id)
{
	...
	case EB_ADDAFTER:
		...
		break;
	case EB_CLEAR:
		if (runConfirmDlg(wiiu_confirm_str(_("Clear level? y/n"))))
			curLevel->clear();
		break;
	case EB_DELETE:
		if (numLevels > 1 && runConfirmDlg(wiiu_confirm_str(_("Delete level? y/n")))) {
	...
	case EB_MOVEDOWN:
		...
		break;
	case EB_QUIT:
		text = wiiu_confirm_str(_("Quit editor? y/n"));
		if (hasChanges)
	...
	case EB_SAVE:
		...
		break;
	case EB_LOAD:
		if (runConfirmDlg(wiiu_confirm_str(_("Reload levelset from file? y/n"))))
			load();
		break;
	case EB_TEST:
		testLevel = true;
